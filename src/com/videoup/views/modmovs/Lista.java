...
public class Lista extends ViewLista{
    ...
    public Lista(Controller ctrl) {
        ...
        initComponents();
        buildTableModel();
        mainQry="select vm.idcm,titulo,director,procmpy,clasif,vct.catg, GROUP_CONCAT(vf.frmt SEPARATOR ', ') AS formt,";
        mainQry+="GROUP_CONCAT(cpy.barcode SEPARATOR ', ') AS bcode, GROUP_CONCAT(cpy.status SEPARATOR ', ') AS status from videoup_movies vm left join ";
        mainQry+="videoup_catgs vct on vm.catg=vct.idcg left join videoup_bcdmov rgc on rgc.idcm=vm.idcm ";
        mainQry+="left join videoup_bcodes cpy on rgc.idbc=cpy.idbc left join videoup_formts vf on cpy.frmt=vf.idcf";
        orderby=" group by vm.idcm order by vm.idcm desc";
        countQry="select count(*) from videoup_movies vm";
        reloadTable(0);
        loadCatgs();
        loadClasifs();
        loadFormats();
    ...
    private void loadClasifs(){
        List<Object> lista;
        lista=loadList("select distinct clasif From VideoupMovies",null,0,false);
    ...
    protected void showList(String query,int pag){
        ...
        for(Object[] row: lista){
            ...
            if(row[8]!=null){
                if( row[8].toString().toLowerCase().contains("disponible") ){
                    ck.addHighLighter("hlGreen",true);
                }else if( row[8].toString().toLowerCase().contains("en alquiler") ){
                    ck.addHighLighter("hlBlue",true);
                }else if( row[8].toString().toLowerCase().indexOf("apartad")>=0 ){
                    ck.addHighLighter("hlOrange",true);
                }else if( row[8].toString().toLowerCase().contains("vendida") ){
    ...
    protected void buildTableModel(){
        tabla.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {},
            new String [] {"Titulo", "Director", "Productora", "Clasificacion", "Genero","Formato(s)","Codigo(s)"}
        ){
    ...
    protected int deleteSelectedRecords(){
        ...
        int selIdxModel;
        cellKey cell;
        String toDel2="delete from videoup_mimgs WHERE;";
        String toDel3="delete from videoup_movies WHERE;";
        String where="";
        DefaultTableModel tmodel=((DefaultTableModel)tabla.getModel());
        int cnt;
        ...
        if(!DBUtils.deleteBCodes(where,true)){ error=DBUtils.getError(); return -1; }
        toDel2=toDel2.replaceAll("WHERE"," where "+where.replaceAll("IDN","idrl"));
        toDel3=toDel3.replaceAll("WHERE"," where "+where.replaceAll("IDN","idcm"));
    ...
    protected void applyFilter(){
        ...
        String dir=txfDirector.getText().trim();
        ...
        if(tit.length()>0){
            ...
            whr+=(whr.length()>0?fltUnion:"")+buildLike4Where(dir,"director");
        }if(!cla.equals("--")){
            whr+=(whr.length()>0?fltUnion:"")+buildLike4Where(cla,"clasif");
        }if(!cat.equals("--")){
            whr+=(whr.length()>0?fltUnion:"")+"vm.catg="+((VideoupCatgs)cat).getIdcg();
            ...
            whr+=(whr.length()>0?fltUnion:"")+"vm.idcpr="+((VideoupCtprrentas)catgAlq).getIdcpr();
        }if(!estado.equals("Cualquiera")){
            whr+=(whr.length()>0?fltUnion:"")+"upper(cpy.status)='"+estado.toUpperCase()+"'";
        }
        if(whr.length()>0){
            whr=" where "+whr;
            countQry="select count(distinct vm.idcm) from videoup_movies vm left join videoup_catgs vct on ";
            countQry+="vm.catg=vct.idcg left join videoup_bcdmov rgc on rgc.idcm=vm.idcm left join videoup_bcodes ";
    ...
    protected void orderByColumn(int col, String ort){
        orderby=" group by vm.idcm order by";
        if(col==0){
            orderby+=" titulo "+ort;
        }else if(col==1){
            orderby+=" director "+ort;
        }else if(col==2){
            orderby+=" procmpy "+ort;
        }else if(col==3){
            orderby+=" clasif "+ort;
    ...
    protected boolean updatePage(String ids,String idn){
        ...
        float precio;
        String upd1="update videoup_movies vm set";
        ...
        if(ctAlq!=null){ upd1+=" vm.idcpr="+ctAlq.getIdcpr(); }
        if(genero!=null){ upd1+=(upd1.endsWith("set")?"":", ")+" vm.catg="+genero.getIdcg(); }
        if(lanz!=null){ upd1+=(upd1.endsWith("set")?"":", ")+" vm.ldate='"+dte2SQL(lanz)+"'"; }
        if(estr!=null){ upd1+=(upd1.endsWith("set")?"":", ")+" vm.estreno_until='"+dte2SQL(estr)+"'"; }
        if(clasif.length()>0){ upd1+=(upd1.endsWith("set")?"":", ")+" vm.clasif='"+clasif+"'"; }
        if(year>0){ upd1+=(upd1.endsWith("set")?"":", ")+" vm.anyo="+year; }
        if(prod.length()>0){ upd1+=(upd1.endsWith("set")?"":", ")+" vm.procmpy='"+prod+"'"; }
    ...
    private String getIdsVb(String ids){
        ...
        qry+="videoup_bcdmov vr, videoup_movies vm where vb.idbc=vr.idbc and vr.idcm=vm.idcm and ("+ids+")";
    ...
    protected void printNow(ArrayList fields){
        PrintDiag pDiag=new PrintDiag(getMainWindow(),true,"Peliculas");
        pDiag.cargaReporte(getParams(), fields,6);
    ...
    private Map<String,Object> getParams(){
        ...
        parametros.put("dreport","Listado de Peliculas");
    ...
    protected ArrayList getListCampos(String query){      
        ...
        for(Object[] row: lista){
            campo=new EntListd( (row[7]!=null?row[7].toString():""),
                    (row[1]!=null?row[1].toString():""),(row[5]!=null?row[5].toString():""),
                    (row[4]!=null?row[4].toString():""),(row[6]!=null?row[6].toString():"") );
    ...
    private void initComponents() {
        ...
        rbtngVendbl = new javax.swing.ButtonGroup();
        ...
        txfDirector.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                txfDirectorKeyPressed(evt);
            }
        });
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        ...
                        .addComponent(txfCBar, javax.swing.GroupLayout.PREFERRED_SIZE, 74, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        ...
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(txfDirector, javax.swing.GroupLayout.PREFERRED_SIZE, 79, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
        ...
        rbtngVendbl.add(rbtnVendbl);
        rbtnVendbl.setText("Si");

        rbtngVendbl.add(rbtnNVendbl);
        rbtnNVendbl.setText("No");

        rbtngVendbl.add(rbtnIgnrVendbl);
        ...
        pnlListEditLayout.setHorizontalGroup(
            ...
            .addGroup(pnlListEditLayout.createSequentialGroup()
                ...
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addGroup(pnlListEditLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(btnEditList, javax.swing.GroupLayout.Alignment.TRAILING)
                    .addComponent(btnGenCred, javax.swing.GroupLayout.Alignment.TRAILING)))
        );
    ...
    private void txfDirectorKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txfDirectorKeyPressed
        if(evt.getKeyCode()==KeyEvent.VK_ENTER){
            applyFilter();
        }
    }//GEN-LAST:event_txfDirectorKeyPressed
    ...
    private void btnEditListActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEditListActionPerformed
        ...
        if(validateEdit()){
            if(ctrl.confirm("Editar registros",ask)){
                useQry="select distinct vm.idcm from videoup_movies vm left join ";
                useQry+="videoup_catgs vct on vm.catg=vct.idcg left join videoup_bcdmov rgc on rgc.idcm=vm.idcm ";
                useQry+="left join videoup_bcodes cpy on rgc.idbc=cpy.idbc left join videoup_formts vf on cpy.frmt=vf.idcf";
                updateRecords(useQry+whr,"vm.idcm=");
            }
        }
    ...
    private void btnGenCredActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGenCredActionPerformed
        String useQry="select distinct cpy.barcode from videoup_movies vm left join videoup_catgs vct on ";
        useQry+="vm.catg=vct.idcg left join videoup_bcdmov rgc on rgc.idcm=vm.idcm join videoup_bcodes ";
    ...
    private javax.swing.ButtonGroup rbtngVendbl;
