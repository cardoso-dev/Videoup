 ...
 * @author Pedro
 */
public class RCardSocios extends ViewReport{
    ...
    /**
     * Creates new form RCardSocios
     */
    public RCardSocios(Controller ctrl, int id) {
        super(ctrl,id,"Credenciales de socios");
        initComponents();
        initComps();
        reloadBCodes=true;
    ...
    private void showTags(ArrayList<TagSingle> allTags, int cols, int rowsSheet){
        ...
        lblInfPreview.setText("Vista previa generada ("+sheets.length+" paginas, "+(cols*rowsSheet)+" credenciales por pagina)");
        validate();
    }
            ...
            for(int i=0;i<items.size();i++){
                ...
                if(codeLst.get(h)!=null){
                    ...
                    if(itm.getItemtype()==2){
                        c_item.setBcValue(codeLst.get(h)[3].toString());
                    }else if(itm.getItemtype()==8){
                        c_item.setText(codeLst.get(h)[0].toString());
                    }else if(itm.getItemtype()==9){
                        c_item.setText(codeLst.get(h)[1].toString());
                    }else if(itm.getItemtype()==10){
                        c_item.setText(codeLst.get(h)[2].toString());
                    }else if(itm.getItemtype()==11){
                        c_item.setText(codeLst.get(h)[3].toString());
                    }else if(itm.getItemtype()==12){
                        c_item.setText( codeLst.get(h)[4]==null?"":codeLst.get(h)[4].toString() );
                    }else if(itm.getItemtype()==13){
                        c_item.setText( codeLst.get(h)[5]==null?"":codeLst.get(h)[5].toString() );
                    }else if(itm.getItemtype()==14){
                        c_item.setText( codeLst.get(h)[6]==null?"":codeLst.get(h)[6].toString() );
                    }else if(itm.getItemtype()==15){
                        c_item.setText( codeLst.get(h)[7]==null?"":codeLst.get(h)[7].toString() );
                    }else if(itm.getItemtype()==16){
                        c_item.setText( codeLst.get(h)[8]==null?"":codeLst.get(h)[8].toString() );
                    }else if(itm.getItemtype()==17){
                        c_item.setText( codeLst.get(h)[9]==null?"":codeLst.get(h)[9].toString() );
                    }else if(itm.getItemtype()==18){
                        c_item.setText( codeLst.get(h)[10]==null?"":codeLst.get(h)[10].toString() );
                    }else if(itm.getItemtype()==19){
                        c_item.setText( codeLst.get(h)[11]==null?"":asDate(codeLst.get(h)[11],DateFormat.SHORT,0) );
                    }else if(itm.getItemtype()==20){
                        c_item.setText( codeLst.get(h)[12]==null?"":asDate(codeLst.get(h)[12],DateFormat.SHORT,0) );
                    }else if(itm.getItemtype()==21){
                        if(codeLst.get(h)[13]!=null){
                            try {
                                blob=((Blob)codeLst.get(h)[13]);
                                c_item.setImage( blob.getBytes(1,(int)blob.length()) );
                                c_item.setType(3);
                            }catch(Exception ex){ c_item.setText("X!"); }
                        }
                    }
    ...
    private void updateBcodes(){
        ...
        counts="select count(cod_cst) from videoup_customers where ";
        cons="select name, applldos, dni, cod_cst, addr, codp, city, prov, pobl, tel_home, tel_movil, ";
        cons+="f_alta, f_vigen, foto from videoup_customers left join videoup_cimgs on idrl=idct where ";
        where=getWhereByCode("cod_cst");
    ...
    private List<VideoupTagitems> getTagsInfo(){
        vtags=(VideoupTags)getEntity("from VideoupTags where idtg=3");
        if(vtags==null){
    ...
    public void insertNullBCode(String bcode, boolean replace){
        ...
        if(codeLst!=null && bcode!=null){
            ...
            for(int h=0;h<tt;h++){
                ...
                if(codeLst.get(h)[3].toString().equals(bcode) && replace){
                    codeLst.set(h, null); break;
                }else if(codeLst.get(h)[3].toString().equals(bcode) && !replace){
    ...
    private void initComponents() {
        ...
        jLabel7.setText("Codigo(s) de socio para imprimir:");
